...
public class UserServiceImpl implements UserService {
	...
	public List<UserDTO> getUserSearchList(String columnName, String value) {
		//쿼리 메소드
		/*
		if(columnName.equals("name")) { // 이름 일때 select * from usertable where name like concat('%' , value ,'%') //findByNameContaining
			return userDAO.findByNameContaining(value);
			
		}else { // 아이디 일때 select *from usertable where id like concat('%','c','%') //else if(columnName.equals("id")) 인데 리턴 때문에 이렇게해줌.
		...
		*/
		...
		if(columnName.equals("name")) { 
			return userDAO.getUserSearchName(value); ///일반메소드
			
		}else { 
			return userDAO.getUserSearchId(value);  	
		}
	}
}
/*
